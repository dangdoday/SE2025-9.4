...
from binancebot.configuration import Configuration
from binancebot.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from binancebot.enums import RPCMessageType, State
from binancebot.exceptions import OperationalException, TemporaryError
from binancebot.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.binancebot = BinanceBot(self._config)
    ...
    def _worker(self, old_state: State | None) -> State:
        ...
        """
        state = self.binancebot.state

        # Log state transition
        if state != old_state:
            if old_state != State.RELOAD_CONFIG:
                self.binancebot.notify_status(f"{state.name.lower()}")
            ...
            if state in (State.RUNNING, State.PAUSED) and old_state not in (
                ...
                self.binancebot.startup()

            if state == State.STOPPED:
                self.binancebot.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.binancebot.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.binancebot.process_stopped()

    def _process_running(self) -> None:
        try:
            self.binancebot.process()
            ...
            self.binancebot.state = State.STOPPED
    ...
    def _reconfigure(self) -> None:
        ...
        self.binancebot.notify_status(f"{State(self.binancebot.state)} after config reloaded")
    ...
    def exit(self) -> None:
        ...
        if self.binancebot:
            self.binancebot.notify_status("process died")
