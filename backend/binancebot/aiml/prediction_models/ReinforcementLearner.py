...
from binancebot.aiml.RL.Base5ActionRLEnv import Actions, Base5ActionRLEnv, Positions
from binancebot.aiml.RL.BaseEnvironment import BaseEnvironment
from binancebot.aiml.RL.BaseReinforcementLearningModel import BaseReinforcementLearningModel
...
class ReinforcementLearner(BaseReinforcementLearningModel):
    ...
    ```
    from binancebot.aiml.prediction_models.ReinforcementLearner import ReinforcementLearner

    class MyCoolRLModel(ReinforcementLearner):
    ```

    Save the file to `user_data/AIMLmodels`, then run it with:

    freqtrade trade --AIMLmodel MyCoolRLModel --config config.json --strategy SomeCoolStrat

    Here the users can override any of the functions
    available in the `IAIMLModel` inheritance tree. Most importantly for RL, this
    is where the user overrides `MyRLEnv` (see below), to define custom
    `calculate_reward()` function, or to override any other parts of the environment.

    This class also allows users to override any other part of the IAIMLModel tree.
    ...
    def fit(self, data_dictionary: dict[str, Any], dk: AIMLDataKitchen, **kwargs):
        ...
        :param dk: AIMLDataKitchen = data kitchen for current pair.
        :return:
        model Any = trained model to be used for inference in dry/live/backtesting
        """
        train_df = data_dictionary["train_features"]
        total_timesteps = self.aiml_info["rl_config"]["train_cycles"] * len(train_df)
        ...
        if dk.pair not in self.dd.model_dictionary or not self.continual_learning:
            model = self.MODELCLASS(
                ...
                **self.aiml_info.get("model_training_parameters", {}),
            )
        else:
            logger.info(
