...
from binancebot.data.history.history_utils import refresh_backtest_ohlcv_data
...
from binancebot.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist
...
def download_all_data_for_training(dp: DataProvider, config: Config) -> None:
    ...
    refresh_backtest_ohlcv_data(
        dp._exchange,
        pairs=all_pairs,
        timeframes=config["AIML"]["feature_parameters"].get("include_timeframes"),
...
def get_required_data_timerange(config: Config) -> TimeRange:
    ...
    for auto data-download in AIML
    """
    ...
    indicator_periods = config["AIML"]["feature_parameters"]["indicator_periods_candles"]
    ...
    logger.info(f"AIML auto-downloader using {max_period} startup candles.")

    additional_seconds = max_period * max_tf_seconds

    startts = int(time - config["AIML"].get("train_period_days", 0) * 86400 - additional_seconds)
    stopts = int(time)
...
def plot_feature_importance(
    model: Any, pair: str, dk: AIMLDataKitchen, count_max: int = 25
) -> None:
    """
    ...
    """
    from binancebot.plot.plotting import go, make_subplots, store_plot_file

    # Extract feature importance from model
    models = {}
    if "AIMLMultiOutputRegressor" in str(model.__class__):
    ...
    for label in models:
        ...
        label = label.replace("&", "").replace("%", "")  # escape two AIML specific characters
...
def get_timerange_backtest_live_models(config: Config) -> str:
    ...
    """
    dk = AIMLDataKitchen(config)
    models_path = dk.get_full_models_path(config)
    dd = AIMLDataDrawer(models_path, config)
...
def get_tb_logger(model_type: str, path: Path, activate: bool) -> Any:
    if model_type == "pytorch" and activate:
        from binancebot.aiml.tensorboard import TBLogger

        return TBLogger(path, activate)
    else:
        from binancebot.aiml.tensorboard.base_tensorboard import BaseTensorboardLogger
