...
from binancebot.data.history import load_pair_history
from binancebot.enums import CandleType
...
class AIMLDataDrawer:
    ...
    def __init__(self, full_path: Path, config: Config):
        self.config = config
        self.aiml_info = config.get("AIML", {})
        ...
        self.model_type = self.aiml_info.get("model_save_type", "joblib")
        ...
        if exists:
            try:
                ...
                with self.historic_predictions_bkp_path.open("rb") as fp:
                    self.historic_predictions = cloudpickle.load(fp)
                logger.warning("AIML successfully loaded the backup historical predictions file.")
    ...
    def set_initial_return_values(
        ...
        zeros during any downtime (between AIML reloads).
        """
        ...
        if len(common_dates.index) > 0:
            ...
            logger.warning(
                ...
                "predictions. You likely left your AIML instance offline "
    ...
    def append_model_predictions(
        ...
        the tail is equivalent to the length of the dataframe that entered AIML from
    ...
    def return_null_values_to_strategy(self, dataframe: DataFrame, dk: AIMLDataKitchen) -> None:
        """
    ...
    def purge_old_models(self) -> None:
        num_keep = self.aiml_info["purge_old_models"]
        if not num_keep:
            return
            ...
            if delete_dict[coin]["num_folders"] > num_keep:
                ...
                for k, v in sorted_dict.items():
                    if deleted >= num_delete:
                        break
                    logger.info(f"AIML purging old model file {v}")
                    shutil.rmtree(v)
                    deleted += 1

    def save_metadata(self, dk: AIMLDataKitchen) -> None:
        """
    ...
    def save_data(self, model: Any, coin: str, dk: AIMLDataKitchen) -> None:
        """
    ...
    def load_metadata(self, dk: AIMLDataKitchen) -> None:
        """
    ...
    def load_data(self, coin: str, dk: AIMLDataKitchen) -> Any:
        """
        ...
        if dk.live and coin in self.model_dictionary:
            ...
            mod = importlib.import_module(
                self.model_type, self.aiml_info["rl_config"]["model_type"]
            )
            MODELCLASS = getattr(mod, self.aiml_info["rl_config"]["model_type"])
    ...
    def update_historic_data(self, strategy: IStrategy, dk: AIMLDataKitchen) -> None:
        """
        ...
        """
        feat_params = self.aiml_info["feature_parameters"]
    ...
    def load_all_pair_histories(self, timerange: TimeRange, dk: AIMLDataKitchen) -> None:
        """
        ...
        for pair in dk.all_pairs:
            if pair not in history_data:
                history_data[pair] = {}
            for tf in self.aiml_info["feature_parameters"].get("include_timeframes"):
    ...
    def get_base_and_corr_dataframes(
        self, timerange: TimeRange, pair: str, dk: AIMLDataKitchen
        ...
        with self.history_lock:
            ...
            pairs = self.aiml_info["feature_parameters"].get("include_corr_pairlist", [])
    ...
    def get_timerange_from_live_historic_predictions(self) -> TimeRange:
        ...
        if not self.historic_predictions_path.is_file():
            raise OperationalException(
                ...
                "to run backtest with the AIML-backtest-live-models option "
            )
