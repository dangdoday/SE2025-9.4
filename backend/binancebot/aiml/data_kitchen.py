...
from binancebot.constants import DOCS_LINK, ORDERFLOW_ADDED_COLUMNS, Config
from binancebot.data.converter import reduce_dataframe_footprint
from binancebot.exceptions import OperationalException
from binancebot.exchange import timeframe_to_seconds
from binancebot.strategy import merge_informative_pair
...
class AIMLDataKitchen:
    """
    Class designed to analyze data for a single pair. Employed by the IAIMLModel class.
    ...
    def __init__(
        ...
        self.config = config
        self.AIML_config: dict[str, Any] = config["AIML"]
        ...
        self.live = live
        self.pair = pair
        self.keras: bool = self.AIML_config.get("keras", False)
        self.set_all_pairs()
        self.backtest_live_models = config.get("AIML_backtest_live_models", False)
        ...
        if not self.live:
            ...
            if not self.backtest_live_models:
                self.full_timerange = self.create_fulltimerange(
                    self.config["timerange"], self.AIML_config.get("train_period_days", 0)
                )
                (self.training_timeranges, self.backtesting_timeranges) = self.split_timerange(
                    self.full_timerange,
                    config["AIML"]["train_period_days"],
                    config["AIML"]["backtest_period_days"],
                )

        self.data["extra_returns_per_train"] = self.AIML_config.get("extra_returns_per_train", {})
        if not self.AIML_config.get("data_kitchen_thread_count", 0):
            self.thread_count = max(int(psutil.cpu_count() * 2 - 2), 1)
        else:
            self.thread_count = self.AIML_config["data_kitchen_thread_count"]
    ...
    def make_train_test_datasets(
        ...
        """
        feat_dict = self.AIML_config["feature_parameters"]

        if "shuffle" not in self.AIML_config["data_split_parameters"]:
            self.AIML_config["data_split_parameters"].update({"shuffle": False})
        ...
        if self.AIML_config.get("data_split_parameters", {}).get("test_size", 0.1) != 0:
            (
                train_features,
                test_features,
                train_labels,
                ...
                labels,
                weights,
                **self.config["AIML"]["data_split_parameters"],
            )
        else:
        ...
        if self.AIML_config["feature_parameters"].get("reverse_train_test_order", False):
    ...
    def set_weights_higher_recent(self, num_weights: int) -> npt.ArrayLike:
        ...
        """
        wfactor = self.config["AIML"]["feature_parameters"]["weight_factor"]
    ...
    def get_predictions_to_append(
        ...
        if self.AIML_config["feature_parameters"].get("DI_threshold", 0) > 0:
    ...
    def create_fulltimerange(self, backtest_tr: str, backtest_period_days: int) -> str:
        ...
        if backtest_timerange.stopts == 0:
            ...
            raise OperationalException(
                "AIML backtesting does not allow open ended timeranges. "
    ...
    def check_if_model_expired(self, trained_timestamp: int) -> bool:
        ...
        max_time = self.AIML_config.get("expiration_hours", 0)
        if max_time > 0:
    ...
    def check_if_new_training_required(
        ...
        timeframes = self.AIML_config["feature_parameters"].get("include_timeframes")
        ...
        if trained_timestamp != 0:
            ...
            retrain = elapsed_time > self.AIML_config.get("live_retrain_hours", 0)
            if retrain:
                trained_timerange.startts = int(
                    time - self.AIML_config.get("train_period_days", 0) * SECONDS_IN_DAY
                )
                ...
                data_load_timerange.startts = int(
                    time
                    - self.AIML_config.get("train_period_days", 0) * SECONDS_IN_DAY
                    - additional_seconds
                )
    ...
    def set_all_pairs(self) -> None:
        self.all_pairs = copy.deepcopy(
            self.AIML_config["feature_parameters"].get("include_corr_pairlist", [])
        )
    ...
    def extract_corr_pair_columns_from_populated_indicators(
        ...
        pairs = self.AIML_config["feature_parameters"].get("include_corr_pairlist", [])
    ...
    def populate_features(
        ...
        """
        tfs: list[str] = self.AIML_config["feature_parameters"].get("include_timeframes")
        ...
        for tf in tfs:
            ...
            for t in self.AIML_config["feature_parameters"]["indicator_periods_candles"]:
            ...
            for n in range(self.AIML_config["feature_parameters"]["include_shifted_candles"] + 1):
                if n == 0:
                    continue
    ...
    def use_strategy_to_populate_indicators(  # noqa: C901
        ...
        if not new_version:
            raise OperationalException(
                ...
                f"{DOCS_LINK}/strategy_migration/#AIML-strategy \n"
                "And the feature_engineering_* documentation: \n"
                f"{DOCS_LINK}/AIML-feature-engineering/"
            )

        tfs: list[str] = self.AIML_config["feature_parameters"].get("include_timeframes")
        pairs: list[str] = self.AIML_config["feature_parameters"].get("include_corr_pairlist", [])
        ...
        corr_pairs: list[str] = self.AIML_config["feature_parameters"].get(
        ...
        if self.live:
            dataframe = strategy.set_AIML_targets(dataframe.copy(), metadata=metadata)
    ...
    def get_full_models_path(self, config: Config) -> Path:
        """
        Returns default AIML model path
        :param config: Configuration dictionary
        """
        AIML_config: dict[str, Any] = config["AIML"]
        return Path(config["user_data_dir"] / "models" / str(AIML_config.get("identifier")))
    ...
    def buffer_timerange(self, timerange: TimeRange):
        ...
        will be NaN and AIML will automatically cut those off of the training
        dataset.
        """
        buffer = self.AIML_config["feature_parameters"]["buffer_train_data_candles"]
        if buffer:
    ...
    def normalize_data(self, data_dictionary: dict) -> dict[Any, Any]:
        ...
        """
        logger.warning(
            f"Your custom IAIMLModel relies on the deprecated"
