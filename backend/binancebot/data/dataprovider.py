...
from binancebot.data.history import get_datahandler, load_pair_history
from binancebot.enums import CandleType, RPCMessageType, RunMode, TradingMode
from binancebot.exceptions import ExchangeError, OperationalException
from binancebot.exchange import Exchange, timeframe_to_prev_date, timeframe_to_seconds
from binancebot.exchange.exchange_types import FundingRate, OrderBook
from binancebot.misc import append_candles_to_dataframe
from binancebot.rpc import RPCManager
from binancebot.rpc.rpc_types import RPCAnalyzedDFMsg
from binancebot.util import PeriodicCache
...
class DataProvider:
    ...
    def get_required_startup(self, timeframe: str) -> int:
        AIML_config = self._config.get("AIML", {})
        if not AIML_config.get("enabled", False):
            return self._config.get("startup_candle_count", 0)
        else:
            startup_candles = self._config.get("startup_candle_count", 0)
            indicator_periods = AIML_config["feature_parameters"]["indicator_periods_candles"]
            # make sure the startupcandles is at least the set maximum indicator periods
            self._config["startup_candle_count"] = max(startup_candles, max(indicator_periods))
            tf_seconds = timeframe_to_seconds(timeframe)
            train_candles = AIML_config["train_period_days"] * 86400 / tf_seconds
            total_candles = int(self._config["startup_candle_count"] + train_candles)
            logger.info(
                f"Increasing startup_candle_count for AIML on {timeframe} to {total_candles}"
            )
