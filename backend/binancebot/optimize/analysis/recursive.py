...
from binancebot.loggers.set_log_levels import (
    reduce_verbosity_for_bias_tester,
    restore_verbosity_for_bias_tester,
)
from binancebot.optimize.analysis.base_analysis import BaseAnalysis, VarHolder
...
class RecursiveAnalysis(BaseAnalysis):
    ...
    def prepare_data(self, varholder: VarHolder, pairs_to_load: list[DataFrame]):
        if "AIML" in self.local_config and "identifier" in self.local_config["AIML"]:
            # purge previous data if the AIML model is defined
            ...
            path_to_current_identifier = Path(
                ...
                f"{self.local_config['AIML']['identifier']}"
            ).resolve()
