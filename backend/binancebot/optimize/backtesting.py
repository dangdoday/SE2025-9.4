...
from binancebot import constants
from binancebot.configuration import TimeRange, validate_config_consistency
from binancebot.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from binancebot.data import history
from binancebot.data.btanalysis import (
    find_existing_backtest_stats,
    get_tick_size_over_time,
    trade_list_to_dataframe,
)
from binancebot.data.converter import trim_dataframe, trim_dataframes
from binancebot.data.dataprovider import DataProvider
from binancebot.data.metrics import combined_dataframes_with_rel_mean
...
from binancebot.exceptions import DependencyException, OperationalException
from binancebot.exchange import (
    amount_to_contract_precision,
    price_to_precision,
    timeframe_to_seconds,
)
from binancebot.exchange.exchange import TICK_SIZE, Exchange
from binancebot.ft_types import (
    BacktestContentType,
    BacktestContentTypeIcomplete,
    BacktestResultType,
    get_BacktestResultType_default,
)
# from binancebot.leverage.liquidation_price import update_liquidation_prices  # Module not available
from binancebot.mixins import LoggingMixin
from binancebot.optimize.backtest_caching import get_strategy_run_id
from binancebot.optimize.bt_progress import BTProgress
from binancebot.optimize.optimize_reports import (
...
from binancebot.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from binancebot.plugins.pairlistmanager import PairListManager
from binancebot.plugins.protectionmanager import ProtectionManager
from binancebot.resolvers import ExchangeResolver, StrategyResolver
...
from binancebot.util import FtPrecise, dt_now
from binancebot.util.migrations import migrate_data
    ...
    def __init__(self, config: Config, exchange: Exchange | None = None) -> None:
        ...
        if self.config.get("strategy_list"):
            if self.config.get("AIML", {}).get("enabled", False):
                logger.warning(
                    "Using --strategy-list with AIML REQUIRES all strategies "
        ...
        if self.config.get("AIML_backtest_live_models", False):
            from binancebot.aiml.utils import get_timerange_backtest_live_models
        ...
        if self.config.get("AIML", {}).get("enabled", False):
            # For AIML, increase the required_startup to includes the training data
    ...
    def _try_close_open_order(
        ...
        if order and self._get_order_filled(order.ft_price, row):
            ...
            if self.margin_mode == MarginMode.CROSS or not (
                ...
                # update_liquidation_prices(  # Disabled - leverage module not available
                #     trade,
                ...
                #     dry_run=True,
                # )
                pass  # Liquidation price update disabled
