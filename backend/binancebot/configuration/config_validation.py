...
from binancebot.config_schema.config_schema import (
    CONF_SCHEMA,
...
from binancebot.configuration.deprecated_settings import process_deprecated_setting
from binancebot.constants import UNLIMITED_STAKE_AMOUNT
from binancebot.enums import RunMode, TradingMode
from binancebot.exceptions import ConfigurationError
...
def validate_config_consistency(conf: dict[str, Any], *, preliminary: bool = False) -> None:
    ...
    _validate_AIML_hyperopt(conf)
    _validate_AIML_backtest(conf)
    _validate_AIML_include_timeframes(conf, preliminary=preliminary)
...
def _validate_edge(conf: dict[str, Any]) -> None:
    ...
    if conf.get("edge", {}).get("enabled"):
        raise ConfigurationError(
            "Edge is no longer supported and has been removed from binancebot with 2025.6."
        )
...
def _validate_AIML_hyperopt(conf: dict[str, Any]) -> None:
    AIML_enabled = conf.get("AIML", {}).get("enabled", False)
    analyze_per_epoch = conf.get("analyze_per_epoch", False)
    if analyze_per_epoch and AIML_enabled:
        raise ConfigurationError(
            "Using analyze-per-epoch parameter is not supported with a AIML strategy."
        )


def _validate_AIML_include_timeframes(conf: dict[str, Any], preliminary: bool) -> None:
    AIML_enabled = conf.get("AIML", {}).get("enabled", False)
    if AIML_enabled:
        main_tf = conf.get("timeframe", "5m")
        AIML_include_timeframes = (
            conf.get("AIML", {}).get("feature_parameters", {}).get("include_timeframes", [])
        )
        ...
        offending_lines = []
        for tf in AIML_include_timeframes:
            tf_s = timeframe_to_seconds(tf)
            if tf_s < main_tf_s:
                offending_lines.append(tf)
        if offending_lines:
            raise ConfigurationError(
                f"Main timeframe of {main_tf} must be smaller or equal to AIML "
                f"`include_timeframes`.Offending include-timeframes: {', '.join(offending_lines)}"
            )

        # Ensure that the base timeframe is included in the include_timeframes list
        if not preliminary and main_tf not in AIML_include_timeframes:
            feature_parameters = conf.get("AIML", {}).get("feature_parameters", {})
            include_timeframes = [main_tf, *AIML_include_timeframes]
            conf.get("AIML", {}).get("feature_parameters", {}).update(
                {**feature_parameters, "include_timeframes": include_timeframes}
            )


def _validate_AIML_backtest(conf: dict[str, Any]) -> None:
    if conf.get("runmode", RunMode.OTHER) == RunMode.BACKTEST:
        AIML_enabled = conf.get("AIML", {}).get("enabled", False)
        timerange = conf.get("timerange")
        AIML_backtest_live_models = conf.get("AIML_backtest_live_models", False)
        if AIML_backtest_live_models and AIML_enabled and timerange:
            raise ConfigurationError(
                "Using timerange parameter is not supported with "
                "--AIML-backtest-live-models parameter."
            )

        if AIML_backtest_live_models and not AIML_enabled:
            raise ConfigurationError(
                "Using --AIML-backtest-live-models parameter is only "
                "supported with a AIML strategy."
            )

        if AIML_enabled and not AIML_backtest_live_models and not timerange:
            raise ConfigurationError(
                "Please pass --timerange if you intend to use AIML for backtesting."
            )
