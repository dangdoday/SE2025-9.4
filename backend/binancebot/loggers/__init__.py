...
from binancebot.loggers.buffering_handler import FTBufferingHandler
from binancebot.loggers.ft_rich_handler import FtRichHandler
from binancebot.loggers.rich_console import get_rich_console


# from binancebot.loggers.std_err_stream_handler import FTStdErrStreamHandler
...
FT_LOGGING_CONFIG = {
    ...
    "handlers": {
        "console": {
            "class": "binancebot.loggers.ft_rich_handler.FtRichHandler",
            "formatter": "basic",
        },
    },
    "root": {
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "binancebot": logging.INFO if verbosity < 1 else logging.DEBUG,
        "binancebot.exchange.exchange_ws": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "binancebot.loggers.ft_rich_handler.FtRichHandler":
