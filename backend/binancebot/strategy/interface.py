...
from binancebot.constants import CUSTOM_TAG_MAX_LENGTH, Config, IntOrInf, ListPairsWithTimeframes
from binancebot.data.converter import populate_dataframe_with_trades
from binancebot.data.converter.converter import reduce_dataframe_footprint
...
from binancebot.strategy.informative_decorator import (
    InformativeData,
    PopulateIndicators,
    _create_and_merge_informative_pair,
    _format_pair_name,
)
from binancebot.strategy.strategy_validation import StrategyResultValidator
from binancebot.strategy.strategy_wrapper import strategy_safe_wrapper
from binancebot.util import dt_now, dt_ts
from binancebot.wallets import Wallets
...
class IStrategy(ABC, HyperStrategyMixin):
    ...
    def load_AIML_model(self) -> None:
        if self.config.get("AIML", {}).get("enabled", False):
            # Import here to avoid importing this if AIML is disabled
            from binancebot.aiml.utils import download_all_data_for_training
            from binancebot.resolvers.AIMLmodel_resolver import AIMLModelResolver

            self.AIML = AIMLModelResolver.load_AIMLmodel(self.config)
            self.aiml_info = self.config["AIML"]
            ...
            # Gracious failures if AIML is disabled but "start" is called.
            class DummyClass:
            ...
            self.AIML = DummyClass()  # type: ignore
    ...
    def populate_any_indicators(
        ...
        This method is *only* used in AIMLDataKitchen class and therefore
        it is only called if AIML is active.
    ...
    def feature_engineering_expand_all(
        ...
        """
        *Only functional with AIML enabled strategies*
        ...
        All features must be prepended with `%` to be recognized by AIML internals.

        More details on how these config defined parameters accelerate feature engineering
        in the documentation at:

        https://www.freqtrade.io/en/latest/AIML-parameter-table/#feature-parameters

        https://www.freqtrade.io/en/latest/AIML-feature-engineering/#defining-the-features
    ...
    def feature_engineering_standard(
        ...
        *Only functional with AIML enabled strategies*
        ...
        AIML_feature_engineering_* functions.
        ...
        https://www.freqtrade.io/en/latest/AIML-feature-engineering
    ...
    def set_AIML_targets(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        """
        *Only functional with AIML enabled strategies*
        Required function to set the targets for the model.
        All targets must be prepended with `&` to be recognized by the AIML internals.
    ...
    def __informative_pairs_AIML(self) -> ListPairsWithTimeframes:
        """
        Create informative-pairs needed for AIML
        """
        if self.config.get("AIML", {}).get("enabled", False):
            whitelist_pairs = self.dp.current_whitelist()
            candle_type = self.config.get("candle_type_def", CandleType.SPOT)
            corr_pairs = self.config["AIML"]["feature_parameters"]["include_corr_pairlist"]
            informative_pairs = []
            for tf in self.config["AIML"]["feature_parameters"]["include_timeframes"]:
    ...
    def gather_informative_pairs(self) -> ListPairsWithTimeframes:
        ...
        informative_pairs.extend(self.__informative_pairs_AIML())
