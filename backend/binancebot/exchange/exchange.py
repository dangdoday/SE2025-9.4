...
from binancebot.constants import (
...
from binancebot.data.converter import (
...
from binancebot.exchange.common import (
    API_FETCH_ORDER_RETRY_COUNT,
    retrier,
    retrier_async,
)
from binancebot.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from binancebot.exchange.exchange_ws import ExchangeWS
from binancebot.misc import (
    chunks,
    deep_merge_dicts,
    file_dump_json,
    file_load_json,
...
from binancebot.util import dt_from_ts, dt_now
from binancebot.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    def validate_timeframes(self, timeframe: str | None) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by BinanceBot.")
    ...
    def validate_AIML(self, config: Config) -> None:
        AIML_enabled = config.get("AIML", {}).get("enabled", False)
        override = config.get("AIML", {}).get("override_exchange_checks", False)
        if not override and AIML_enabled and not self._ft_has["ohlcv_has_history"]:
            raise ConfigurationError(
                f"Historic OHLCV data not available for {self.name}. Can't use aiml."
            )
        elif override and AIML_enabled and not self._ft_has["ohlcv_has_history"]:
            logger.warning(
                "Overriding exchange checks for aiml. Make sure that your exchange supports "
                "fetching historic OHLCV data, otherwise AIML will not work."
            )
    ...
    def fetch_dry_run_order(self, order_id: str) -> CcxtOrder:
        ...
        try:
            ...
            from binancebot.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("BinanceBot does not yet support inverse contracts")
            ...
            raise OperationalException(
                "BinanceBot only supports isolated futures for leverage trading"
            )
