...
from binancebot.constants import CANCEL_REASON, DEFAULT_DATAFRAME_COLUMNS, Config
from binancebot.data.history import load_data
from binancebot.data.metrics import DrawDownResult, calculate_expectancy, calculate_max_drawdown
from binancebot.enums import (
    CandleType,
    ExitCheckTuple,
    ExitType,
    MarketDirection,
...
from binancebot.plugins.pairlist.pairlist_helpers import expand_pairlist
from binancebot.rpc.fiat_convert import CryptoToFiatConverter
from binancebot.rpc.rpc_types import RPCSendMsg
from binancebot.util import (
    decimals_per_coin,
    dt_from_ts,
    dt_humanize_delta,
    dt_now,
...
from binancebot.wallets import PositionWallet, Wallet
...
class RPC:
    ...
    if TYPE_CHECKING:
        ...
        _binancebot: BinanceBot

    def __init__(self, binancebot) -> None:
        """
        Initializes all enabled rpc modules
        :param binancebot: Instance of a binancebot bot
        :return: None
        """
        self._binancebot = binancebot
        self._config: Config = binancebot.config
    ...
    def _rpc_trade_status(self, trade_ids: list[int] | None = None) -> list[dict[str, Any]]:
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.is_open:
                    try:
                        current_rate: float = self._binancebot.exchange.get_rate(
                ...
                if not isnan(current_profit_abs) and self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._binancebot.config["stake_currency"],
                        self._binancebot.config["fiat_display_currency"],
                    )
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._binancebot.wallets.get_starting_balance()
    ...
    def __balance_get_est_stake(
        ...
        if coin == self._binancebot.exchange.get_proxy_coin():
            ...
            try:
                rate: float | None = self._binancebot.exchange.get_conversion_rate(
                    coin, stake_currency
                )
                if rate:
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> dict:
        ...
        self._binancebot.wallets.update(require_update=False)
        starting_capital = self._binancebot.wallets.get_starting_balance()
        starting_cap_fiat = (
            self._fiat_converter.convert_amount(
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._binancebot.wallets.get_all_balances().items():
            if not balance.total and not balance.free:
                continue

            trade = (
                open_assets.get(coin, None)
                if self._binancebot.trading_mode != TradingMode.FUTURES
                else None
            )
            is_stake_currency = coin == self._binancebot.exchange.get_proxy_coin()
            is_bot_managed = is_stake_currency or trade is not None
            trade_amount = trade.amount if trade else 0
            if is_stake_currency:
                trade_amount = self._binancebot.wallets.get_available_stake_amount()
        ...
        for symbol, position in self._binancebot.wallets.get_all_positions().items():
        ...
        return {
            ...
            "note": "Simulated balances" if self._binancebot.config["dry_run"] else "",
        }

    def _rpc_start(self) -> dict[str, str]:
        """Handler for start"""
        if self._binancebot.state == State.RUNNING:
            return {"status": "already running"}

        self._binancebot.state = State.RUNNING
        return {"status": "starting trader ..."}

    def _rpc_stop(self) -> dict[str, str]:
        """Handler for stop"""
        if self._binancebot.state != State.STOPPED:
            self._binancebot.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> dict[str, str]:
        ...
        self._binancebot.state = State.RELOAD_CONFIG
    ...
    def _rpc_pause(self) -> dict[str, str]:
        ...
        """
        if self._binancebot.state == State.RUNNING:
            self._binancebot.state = State.PAUSED
    ...
    def __exec_force_exit(
        ...
        if all(tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry):
            ...
            order_type = ordertype or self._binancebot.strategy.order_types.get(
                "force_exit", self._binancebot.strategy.order_types["exit"]
            )
            sub_amount: float | None = None
            if amount and amount < trade.amount:
                # Partial exit ...
                min_exit_stake = self._binancebot.exchange.get_min_pair_stake_amount(
    ...
    def _rpc_force_exit(
        ...
        with self._binancebot._exit_lock:
            if trade_id == "all":
                ...
                Trade.commit()
                self._binancebot.wallets.update()
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        if not self._binancebot.config.get("force_entry_enable", False):
            raise RPCException("Force_entry not enabled.")

        if self._binancebot.state != State.RUNNING:
            raise RPCException("trader is not running")

        if order_side == SignalDirection.SHORT and self._binancebot.trading_mode == TradingMode.SPOT:
            raise RPCException("Can't go short on Spot markets.")

        if pair not in self._binancebot.exchange.get_markets(tradable_only=True):
            raise RPCException("Symbol does not exist or market is not active.")
        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._binancebot.config.get("stake_currency")
        if not self._binancebot.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
    ...
    def _rpc_force_entry(
        ...
        *,
        ...
        if trade:
            ...
            if not self._binancebot.strategy.position_adjustment_enable:
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._binancebot.wallets.get_trade_stake_amount(
                pair, self._config["max_open_trades"]
            )

        # execute buy
        if not order_type:
            order_type = self._binancebot.strategy.order_types.get(
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._binancebot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    order = self._binancebot.exchange.fetch_order(open_order.order_id, trade.pair)
                except ExchangeError as e:
                    logger.info(f"Cannot query order for {trade} due to {e}.", exc_info=True)
                    raise RPCException("Order not found.")
                self._binancebot.handle_cancel_order(
    ...
    def _rpc_delete(self, trade_id: int) -> dict[str, str | int]:
        ...
        with self._binancebot._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._binancebot.exchange.cancel_order(open_order.order_id, trade.pair)
                    c_count += 1
            ...
            if (
                self._binancebot.strategy.order_types.get("stoploss_on_exchange")
                and trade.has_open_sl_orders
            ):
                for oslo in trade.open_sl_orders:
                    try:
                        self._binancebot.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                        c_count += 1
                    except ExchangeError:
                        pass
    ...
    def _rpc_count(self) -> dict[str, float]:
        ...
        return {
            "current": len(trades),
            "max": (
                int(self._binancebot.config["max_open_trades"])
                if self._binancebot.config["max_open_trades"] != float("inf")
                else -1
            ),
    ...
    def _rpc_whitelist(self) -> dict:
        ...
        res = {
            "method": self._binancebot.pairlists.name_list,
            "length": len(self._binancebot.active_pair_whitelist),
            "whitelist": self._binancebot.active_pair_whitelist,
        }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: list[str]) -> dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._binancebot.pairlists.blacklist:
                self._binancebot.pairlists.blacklist.remove(pair)
            else:
    ...
    def _rpc_blacklist(self, add: list[str] | None = None) -> dict:
        ...
        if add:
            for pair in add:
                if pair not in self._binancebot.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], list(self._binancebot.exchange.get_markets().keys()))
                        self._binancebot.pairlists.blacklist.append(pair)
        ...
        res = {
            "method": self._binancebot.pairlists.name_list,
            "length": len(self._binancebot.pairlists.blacklist),
            "blacklist": self._binancebot.pairlists.blacklist,
            "blacklist_expanded": self._binancebot.pairlists.expanded_blacklist,
            "errors": errors,
        }
        return res
    ...
    def _rpc_analysed_dataframe(
        ...
        annotations = self._binancebot.strategy.ft_plot_annotations(pair=pair, dataframe=_data)

        return RPC._convert_dataframe_to_dict(
            self._binancebot.config["strategy"],
            pair,
            timeframe,
            _data,
            last_analyzed,
    ...
    def __rpc_analysed_dataframe_raw(
        ...
        """
        _data, last_analyzed = self._binancebot.dataprovider.get_analyzed_dataframe(pair, timeframe)
        _data = _data.copy()
    ...
    def _ws_all_analysed_dataframes(
        ...
        """
        timeframe = self._binancebot.config["timeframe"]
        candle_type = self._binancebot.config.get("candle_type_def", CandleType.SPOT)
    ...
    def _rpc_analysed_history_full(
        ...
        from binancebot.data.converter import trim_dataframe
        from binancebot.data.dataprovider import DataProvider
        from binancebot.persistence.usedb_context import FtNoDBContext
    ...
    def _rpc_plot_config(self) -> dict[str, Any]:
        if (
            self._binancebot.strategy.plot_config
            and "subplots" not in self._binancebot.strategy.plot_config
        ):
            self._binancebot.strategy.plot_config["subplots"] = {}
        return self._binancebot.strategy.plot_config
    ...
    def health(self) -> dict[str, str | int | None]:
        last_p = self._binancebot.last_process
    ...
    def _update_market_direction(self, direction: MarketDirection) -> None:
        self._binancebot.strategy.market_direction = direction

    def _get_market_direction(self) -> MarketDirection:
        return self._binancebot.strategy.market_direction
